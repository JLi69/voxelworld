...
fn add_face_transparent(
    ...
    let adj_z = wrap_coord(z + offz) as usize;
    if let Some(adj_chunk) = adj_chunk {
        let show_face = (adj_chunk
            .get_block_relative(adj_x, adj_y, adj_z)
            .transparent()
            && adj_chunk.get_block_relative(adj_x, adj_y, adj_z).id != blockid)
            || (adj_chunk
                .get_block_relative(adj_x, adj_y, adj_z)
                .transparent()
                && adj_chunk.get_block_relative(adj_x, adj_y, adj_z).id == blockid
    ...
    let adj_y = (y + offy) as usize;
    let adj_z = (z + offz) as usize;
    let show_face = (chunk.get_block_relative(adj_x, adj_y, adj_z).transparent()
        && chunk.get_block_relative(adj_x, adj_y, adj_z).id != blockid)
        || (chunk.get_block_relative(adj_x, adj_y, adj_z).transparent()
            && chunk.get_block_relative(adj_x, adj_y, adj_z).id == blockid
            && !chunk.get_block_relative(adj_x, adj_y, adj_z).can_connect());
